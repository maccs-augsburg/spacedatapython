...
# Entry Check file moved from view_maccsGUI.py by Chris Hance 
...
from plot_three_axis_graphs import x_y_and_z_plot,plot_two_axis,plot_axis
from view_maccsGUI import MainWindow
...
def year_day_entry_check( self,year_day_value):
    ...
    if (len(year_day_value) == 0):
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in Year Day box \nValue invalid; Most likely empty\nPlease make sure value on file is correct")
...
def station_names_entry_check(self,station_names_value):
    ...
    if(len(station_names_value) == 0):
        ...
        MainWindow.warning_message_pop_up(self,"Station Code Error", "Invalid Station Code on file \nPlease make sure you select proper file \nExiting program")
...
def file_format_entry_check(self,selection_file_value):
    ...
    if(selection_file_value == '1'):
        ...
        MainWindow.warning_message_pop_up(self, "File format option error", "Sorry! But we don't have this option available yet, please try picking a different option")
    ...
    # Otherwise we can assume that no option had been selected
        ...
        MainWindow.warning_message_pop_up(self,"File open error", "Couldn't find and open your file \nPlease make sure you select proper file \nPlease try again.")
...
def graph_from_plotter_entry_check(self,graph_from_plotter_value_x,graph_from_plotter_value_y, graph_from_plotter_value_z, xArr, yArr, zArr, timeArr, filename, stime, etime, selection_file,zoom, left_xlim, right_xlim):
    ...
    # For single axis plotting
        ...
        if (x_state):
            fig = plot_axis(xArr, timeArr, filename, stime, etime, 'X',zoom, left_xlim, right_xlim)
        
        if(y_state):
            fig = plot_axis(yArr, timeArr, filename, stime, etime, 'Y',zoom, left_xlim, right_xlim)
        
        if(z_state):
            fig = plot_axis(zArr, timeArr, filename, stime, etime, 'Z',zoom, left_xlim, right_xlim)
            
    #Warning Message
    else:
        MainWindow.warning_message_pop_up(self,"Axis Selection",  "Please select an axis\' to plot")
...
def start_hour_entry_check(self,start_hour_string):
    ...
    if(value > 23):
        ...
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in Start Hour box \nValue too large\nPlease make sure value is less than 24")
...
def start_minute_entry_check(self,start_minute_string):
    ...
    if(value > 59):
        ...
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in Start Min box \nValue too large\nPlease make sure value is less than 60")
...
def start_second_entry_check(self,start_second_string):
    ...
    if(value > 59):
        # Have error messsage box pop up becuase it can't be more than 59
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in Start Second box \nValue too large\nPlease make sure value is less than 60")
...
def end_hour_entry_check(self,end_hour_string):
    ...
    if(value > 24):
        # Have error message box pop up because it can't be more than 23
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in End Hour box \nValue too large\nPlease make sure value is less than 24")
...
def end_minute_entry_check(self,end_minute_string):
    ...
    if(value > 59):
        #Have error messsage box pop up becuase it can't be more than 59
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in End Min box \nValue too large\nPlease make sure value is less than 60")
    # Testing to see if the inputted value is less than what it can be
    elif(value < 0):
        # Have error message box pop up because it can't be a negative number
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in End Min box \nValue too small\nPlease make sure value is greater or equal to 0")
...
def end_second_entry_check(self,end_second_string):
    ...
    if(value > 59):
        # Have error messsage box pop up becuase it can't be more than 59
        MainWindow.warning_message_pop_up(self,"Time Entry Error", "Invalid time entry in End Sec box \nValue too large\nPlease make sure value is less than 60")

    # Testing to see if the inputted value is less than what it can be
    elif(value < 0):
        # Have error message box pop up because it can't be a negative number
        MainWindow.warning_message_pop_up(self,"File open error", "Invalid time entry in End Sec box \nValue too small\nPlease make sure value is greater or equal to 0")
