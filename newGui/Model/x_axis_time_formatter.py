...
def create_time_list(stime, etime):
	...
	if (stime == datetime.time.fromisoformat( "00:00:00") and etime == datetime.time.fromisoformat('23:59:59')):
		return default_ticks(stime, etime)
	...
	if hour_step is not None:
		return hour_ticks(hour_step, stime, etime, total_time_diff_seconds)
	...
	if minute_step is not None:
		return minute_ticks(minute_step, stime, etime, total_time_diff_seconds)
	...
	if second_step is not None:
		return second_ticks(second_step, stime, etime, total_time_diff_seconds)


def default_ticks(stime, etime):
...
def hour_ticks(hour_step, stime, etime, total_time_diff_seconds):
	...
	for i in range(int(total_time_diff_seconds / SEC_PER_HOUR) + 1):
		...
		# Only adding the hour to the hours_arr if it is the same as the odd_or_even
		# This is how Ted had it, up to me I would take it out
...
def second_ticks(second_step, stime, etime, total_time_diff_seconds):
